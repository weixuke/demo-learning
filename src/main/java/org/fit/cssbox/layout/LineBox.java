...
public class LineBox
{
    ...
    private int computeBaselineDifference(InlineElement box)
    {
        int a = box.getBaselineOffset();
        int b = box.getBelowBaseline();
        VerticalAlign va = box.getVerticalAlign();
        
        int dif = 0;

        if (va == VerticalAlign.BASELINE)
            dif = 0; //just sits on the baseline
        else if (va == VerticalAlign.MIDDLE)
        {
            int midbox = (a + b) / 2;
        ...
        else if (va == VerticalAlign.SUB)
            dif = (int) Math.round(0.3 * parent.getLineHeight());
        else if (va == VerticalAlign.SUPER)
            dif = - (int) Math.round(0.3 * parent.getLineHeight());  
        else if (va == VerticalAlign.TEXT_TOP)
        {
            int na = parent.getVisualContext().getBaselineOffset();
            dif = a - na;
        }
        else if (va == VerticalAlign.TEXT_BOTTOM)
        {
            int nb = parent.getVisualContext().getFontHeight() - parent.getVisualContext().getBaselineOffset();
            dif = nb - b;
        }
        else if (va == VerticalAlign.length || va == VerticalAlign.percentage)
        {
